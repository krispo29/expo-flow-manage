...
import { Loader2, Search, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, CheckSquare, RefreshCw, Printer, CheckCircle2, XCircle, Clock, Building2, Filter, X, Download } from 'lucide-react'
import {
  Card,
  CardContent,
  CardDescription,
        ...
        <CardContent className="p-0">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between p-6 gap-4 border-t border-white/5 bg-white/5">
            <div className="text-sm text-muted-foreground italic font-medium">
              Streaming <span className="text-foreground">{total > 0 ? (page - 1) * limit + 1 : 0}</span> to <span className="text-foreground">{Math.min(page * limit, total)}</span> of <span className="text-foreground font-bold">{total}</span> artifacts
            </div>
