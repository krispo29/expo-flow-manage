...
export function BMExhibitorReadyCampaignCard({
  projectId,
  readyCount,
  onBatchExecuted,
}: BMExhibitorReadyCampaignCardProps) {
  const [loading, setLoading] = useState(false)
  const [actionLoading, setActionLoading] = useState(false)
  const [campaign, setCampaign] =
    useState<BMExhibitorReadyCampaignStatus | null>(null)
  ...
  const handleStart = async () => {
    ...
    const res = await startBMExhibitorCampaign(
      projectId,
      batchSize,
      intervalMinutes
    )
    ...
    if (res.success) {
      toast.success(
        `Scheduled exhibitor campaign started! Sends ${batchSize} emails every ${intervalMinutes} mins.`
      )
      fetchStatus()
          ...
          <div className="flex shrink-0 items-center gap-3">
            ...
            <div>
              <h3 className="text-foreground hidden text-sm font-bold sm:block">
                Business Matching Exhibitor Ready Email
              </h3>
                ...
                <div className="bg-card border-border/60 flex flex-1 flex-col justify-between gap-2 rounded-xl border p-3.5 shadow-xs">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        setSendingTest(true)
                        const res = await sendTestBMExhibitorCampaign(
                          projectId,
                          testEmail
                        )
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      setRetrying(true)
                      const res =
                        await retryFailedBMExhibitorCampaign(projectId)
                      setRetrying(false)
                      if (res.success) {
