...
jest.mock('@/lib/server-auth', () => ({
  requireServerAuthHeaders: jest.fn().mockResolvedValue({ Authorization: 'Bearer token' }),
}))
...
test('BM campaign actions permit THAILAB2026', async () => {
  ...
  await expect(getBMExhibitorCampaignStatus(THAILAB2026_PROJECT_UUID)).resolves.toMatchObject({ success: true })
  await expect(startBMVisitorCampaign(THAILAB2026_PROJECT_UUID)).resolves.toMatchObject({ success: true })
